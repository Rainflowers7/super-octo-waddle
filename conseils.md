...
Cela peut paraître évident, mais il faut connaître un minimum jeu de base afin de pouvoir apprécier le Randomizer. Si vous découvrez le jeu de cette manière, vous risquez de galérer plus qu'autre chose.

## Le Tracker

Le Tracker est l'outil le plus pratique pour un débutant, puisqu'il vous permet de faire plusieurs choses :

- Connaître la position des localisations ou "checks"
- Voir quels objets vous avez à votre disposition
- Quels checks sont disponibles pour vous avec vos objets actuels
- Quels checks vous avez déjà fait

Comme toute chose, il doit être configuré, mais la tâche est très facile d'accès. Vous vous souvenez peut-être du "Spoiler Log" qui est proposé au téléchargement avec votre monde ?

1) Allez dans File -> New -> Upload Spoiler
2) Cliquez sur "Load Spoiler", sélectionnez le fichier.
3) Si vous jouez à plusieurs, renseignez votre ID de joueur dans "Your Multiworld World". Si vous jouez seul, ignorez cette étape.
4) Cliquez sur Submit.

Si des endroits de votre carte s'allument en jaune, vous avez réussi !

Vous pouvez également changer votre âge de départ dans les paramètres du tracker.

1) Cliquez sur Randomizer Options dans le bandeau en haut de l'écran.
2) Cherchez l'option "Starting Age" -> renseignez l'âge de Link (Adult ou Child)

Enfin, renseignez si vos donjons sont Vanilla (V) ou Master Quest (MQ) dans l'encadrement réservé aux donjons.
...
[Voir le glitch en action est peut-être plus parlant ?](https://www.youtube.com/watch?v=giJT7FWSjBc)
...
Ce token est, dans 90% des cas, la raison pour laquelle vous faites un deuxième voyage dans la caverne Dodongo. Cependant, vous pouvez utiliser un missile teigneux ou tirer sur l'araignée (avec un arc, un lance-pierres ou un grappin) pour voir ce qui s'y cache. Si vous êtes en adulte, l'amélioration du grappin peut l'atteindre sans avoir à revenir dans le donjon une deuxième fois. S'il s'agit d'un objet important, finissez le donjon puis revenez via l'ascenseur de la salle principale (en vous assurant de l'avoir activé au préalable !), sinon vous pouvez ignorer cette localisation.
